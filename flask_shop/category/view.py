...
def get_category_list():
    ...
    cate_list = []
    basequery = models.Category.query.filter(models.Category.level == 1)

    if all([pnum,psize]):
        categories = basequery.paginate(pnum,psize)
        if level:
            cate_list = get_tree(categories.item, level, True)
        else:
